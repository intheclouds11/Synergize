...
namespace HurricaneVR.Framework.Weapons.Guns
{
    public class HVRGunBase : HVRDamageProvider
    {
        ...
        public float SparksDestroyTime = 1.3f;
        ...
        public void Enable(bool enable, bool addedToInventory = false)
        {
            ...
            if (Ammo) Ammo.gunEnabled = enable;
        ...
        protected virtual void CheckDiscardActivated()
        {
            if (Grabbable.IsHandGrabbed && HVRManager.Instance.PlayerController.Inputs.IsDiscardActivated)
            {
                DiscardAbilityPressed();
            }
        }
        
        public virtual void DiscardAbilityPressed()
        {
            SFXPlayer.Instance.PlaySFX(GunSounds.Discarded, HVRManager.Instance.Camera.position, 1f, 1f, 10, false, false);
        ...
        protected virtual void PlayHaptics(HVRGrabbable grabbable, HapticData data)
        {
            ...
            data.Frequency = originalFrequency;
        }
        ...
        protected virtual void OnHit(RaycastHit hit, Vector3 direction)
        {
            ...
            {
                var sparks = Instantiate(HitSparks, hit.point, Quaternion.identity);
                if (sparks)
                {
                    var timer = sparks.GetComponent<HVRDestroyTimer>();
                    if (timer)
                    {
                        timer.StartTimer(SparksDestroyTime);
                    }
                }
            }
        ...
        {
            if (IsBulletChambered || !RequiresChamberedBullet)
            {
                IsBulletChambered = false;
                DisableChamberedRound();
